...
const CGFloat JDSideMenuDefaultDamping = 0.5;//0.7

// animation times
const CGFloat JDSideMenuDefaultOpenAnimationTime = 0.3;//0.3
const CGFloat JDSideMenuDefaultCloseAnimationTime = 0.15;//0.15

@interface JDSideMenu ()
@property (nonatomic, strong) UIImageView *backgroundView;
@property (nonatomic, strong) ShadowView *containerView;
//@property (nonatomic, strong) UITapGestureRecognizer *tapRecognizer;//vane.公开
//@property (nonatomic, strong) UIPanGestureRecognizer *panRecognizer;
@end

@implementation JDSideMenu
{
    UIView *topGroundView;//灰色背景.vane
}
                 ...
                 menuController:(UIViewController*)menuController;
{
    ...
    if (self) {
        ...
        //灰色背景.vane
        topGroundView = [[UIView alloc] initWithFrame:[UIScreen mainScreen].bounds];
        [topGroundView setBackgroundColor:RGBA(255, 255, 0, 0)];
...
- (void)viewDidLoad;
{
    ...
    // add subviews
    _containerView = [[ShadowView alloc] initWithFrame:self.view.bounds];//加阴影.vane
    ...
    [self.tapRecognizer setCancelsTouchesInView:NO];//取消手势独占.vane
...
- (void)tapRecognized:(UITapGestureRecognizer*)recognizer
{
    ...
    if (![self isMenuVisible]) {
        //[self showMenuAnimated:YES];//vane
    } else {
...
- (void)panRecognized:(UIPanGestureRecognizer*)recognizer
{
    ...
    switch (recognizer.state) {
        case UIGestureRecognizerStateBegan: {
            _tapGestureEnabled = NO;//vane.解决“手势共存”引起的tap和pan手势混乱，并导致闪白情况
        ...
        case UIGestureRecognizerStateChanged: {
            ...
            [self.delegate starPan:self];//vane
//            [self statusBarView].transform = recognizer.view.transform;
        ...
        case UIGestureRecognizerStateCancelled: {
            if (velocity.x > 5.0 || (velocity.x >= -1.0 && translation.x > JDSideMenuMinimumRelativePanDistanceToOpen*self.menuWidth)) {
                ...
                [self hideMenuAnimated:YES];
                [self.delegate stopPan:self];//vane
            }
            _tapGestureEnabled = YES;//vane.解决“手势共存”引起的tap和pan手势混乱，并导致闪白情况
        }
        default:
            break;
    }
         ...
         initialVelocity:(CGFloat)velocity;
{
    ...
    // animate
//    __weak typeof(self) blockSelf = self;
//    [UIView animateWithDuration:animated ? duration : 0.15 delay:0
...
//             [_containerView addSubview:topGroundView];//添加灰色背景.vane
//         } completion:nil];
...
{
    [topGroundView removeFromSuperview];//移除灰色背景.vane
