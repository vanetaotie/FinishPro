...
//    nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%@年%zu月%zu日 %@ %@:%@:%@",[dateString substringWithRange:NSMakeRange(0, 4)],monthInt,dayInt,weekString,[dateString substringWithRange:NSMakeRange(8, 2)],[dateString substringWithRange:NSMakeRange(10, 2)],[dateString substringWithRange:NSMakeRange(12, 2)]];
//    [self addSubview:nowPickerShowTimeLabel];
//    datePickerValue = [NSDate date];
    ...
    [middleSepView setBackgroundColor:RGBA(28.0, 93.0, 176.0, 1.0)];
...
- (UIView *)pageAtIndex:(NSInteger)index andScrollView:(MXSCycleScrollView *)scrollView
{
    ...
    {
        if (index < 10) {
            l.text = [NSString stringWithFormat:@"0%zu",(long)index];
        }
        else
            l.text = [NSString stringWithFormat:@"%zu",(long)index];
    }
    else if (scrollView == minuteScrollView)
    {
        if (index < 10) {
...
- (void)selectSetBroadcastTime
{
    ...
    NSString *taskDateString = [NSString stringWithFormat:@"%zu%02zu%02zu%02zu%02zu%02zu",yearInt,monthInt,(long)dayInt,(long)hourInt,(long)minuteInt,(long)secondInt];
...
- (void)scrollviewDidChangeNumber
{
    ...
    NSInteger minuteInt = minuteLabel.tag - 1;
    NSInteger secondInt = secondLabel.tag - 1;
//    NSString *dateString = [NSString stringWithFormat:@"%zu%02zu%02zu%02zu%02zu%02zu",yearInt,monthInt,dayInt,hourInt,minuteInt,secondInt];
//    NSString *weekString = [self fromDateToWeek:dateString];
//    nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%zu年%zu月%zu日 %@ %02zu:%02zu:%02zu",yearInt,monthInt,dayInt,weekString,hourInt,minuteInt,secondInt];
//    
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    NSString *selectTimeString = [NSString stringWithFormat:@"%zu-%02zu-%02zu %02zu:%02zu:%02zu",yearInt,monthInt,(long)dayInt,(long)hourInt,(long)minuteInt,(long)secondInt];
...
//        selectTimeIsNotLegalLabel.text = @"温馨提示：所选时间不合法，无法提交";
        selectTimeIsNotLegalLabel.text = @"提示：所选时间为已过去时间";//vane修改
//        [OkBtn setEnabled:NO];
    }
    else
    {
...
//        [OkBtn setEnabled:YES];
    ...
    datePickerValue = selectDate; //传值.vane增加
    [self.delegate changeDatePickerValue:datePickerValue];//vane增加
}
//通过日期求星期
